  ...
  try {
    ...
    for (const node of ast.body) {
      if (
        ...
        if (
          actualNodeToCommentCheck &&
          reportedNodeForMissingDoc &&
          exportType !== "reference" &&
          exportType !== "variable"
        ) {
          // Skip 'reference' type and non-function/class variables for now
          let hasJSDoc = false;
          // leadingComments are attached to the node itself by the parser when { comment: true }
          if (
            actualNodeToCommentCheck.leadingComments &&
            actualNodeToCommentCheck.leadingComments.length > 0
          ) {
            for (const comment of actualNodeToCommentCheck.leadingComments) {
              if (comment.type === "Block" && comment.value.startsWith("*")) {
                ...
                const textBetween = code.substring(
                  comment.range[1],
                  actualNodeToCommentCheck.range[0],
                );
                if (textBetween.trim() === "") {
                  hasJSDoc = true;
                  break;
          ...
          if (!hasJSDoc) {
            missingDocs.push(
              `- ${exportType} '${name}' in ${path.basename(filePath)}`,
            );
          }
        }
      }
    ...
    if (error.message.includes("Parsing error")) {
      missingDocs.push(
        `- Could not parse file ${path.basename(filePath)}. Ensure it's valid TypeScript/JavaScript.`,
      );
    } else {
      missingDocs.push(
        `- Error processing file ${path.basename(filePath)} (see console for details).`,
      );
    }
  }
  return missingDocs;
