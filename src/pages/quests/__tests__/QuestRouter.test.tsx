...
vi.mock("../QuestPlaceholder", () => ({
  default: ({ id }: { id: number }) => (
    <div data-testid="quest-placeholder">QuestPlaceholder {id}</div>
  ),
}));
...
describe("QuestRouter", () => {
  it("renders QuestPlaceholder with valid quest ID", () => {
    ...
    expect(screen.getByTestId("quest-placeholder")).toHaveTextContent(
      "QuestPlaceholder 1",
    );
  });
      ...
      <MemoryRouter initialEntries={["/quest/invalid"]}>
        <Routes>
          ...
          <Route
            path="/quest/0"
            element={
              <div data-testid="quest-placeholder">QuestPlaceholder 0</div>
            }
          />
        </Routes>
      </MemoryRouter>,
